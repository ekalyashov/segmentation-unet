...
def center_cropping_resize(img, new_size):
    ...
    new_largest = max(new_size[0], new_size[1])
    new_h = np.round(np.multiply(new_largest / largest, img.size[1])).astype(int)
    new_w = np.round(np.multiply(new_largest / largest, img.size[0])).astype(int)
    return img.resize((new_w, new_h), Image.BICUBIC)

def get_center_crop_size(img_path, img_size):
    img = Image.open(img_path)
    largest = max(img.width, img.height)
    new_largest = max(img_size[0], img_size[1])
    new_h = np.round(np.multiply(new_largest / largest, img.size[1])).astype(int)
    new_w = np.round(np.multiply(new_largest / largest, img.size[0])).astype(int)
    return new_w, new_h